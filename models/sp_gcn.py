...
layers = utils.layers.sp_gcn_head
...
class SpGCN(BaseGAttN):
    def __init__(self,nb_classes, nb_nodes, training, attn_drop, ffd_drop, nnz,in_sz,
        ...
        for _ in range(n_heads[0]):
            self.attns.append(layers(in_sz=in_sz,
        ...
        for i in range(1, len(hid_units)):
            ...
            for _ in range(n_heads[i]):
                internal_attns.append(layers(in_sz=hid_units[i-1],  
                adj_mat=bias_mat, nnz=nnz,
        ...
        for i in range(n_heads[-1]):
            self.out.append(layers(in_sz=hid_units[-1],adj_mat=bias_mat, nnz=nnz,
