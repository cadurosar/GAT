...
class MLP(BaseGAttN):
    def __init__(nb_classes, nb_nodes, training, attn_drop, ffd_drop, nnz,
            bias_mat, hid_units, n_heads, activation=torch.nn.ELU,
            intra_drop=None, intra_activation=None, scheme_norm=None, scheme_init_std=None,
            residual=False, use_bias=True):
        attns = []
        for _ in range(n_heads[0]):
            attns.append(layers.mlp_head(inputs,
        ...
        for i in range(1, len(hid_units)):
            with tf.variable_scope('level_' + str(i)):
                ...
                for _ in range(n_heads[i]):
                    with tf.variable_scope('head_' + str(_)):
                      attns.append(layers.mlp_head(h_1,  
                        adj_mat=bias_mat, nnz=nnz,
        ...
        with tf.variable_scope('level_' + str(len(hid_units))):
            for i in range(n_heads[-1]):
                with tf.variable_scope('head_' + str(i)):
                  out.append(layers.mlp_head(h_1, adj_mat=bias_mat, nnz=nnz,
